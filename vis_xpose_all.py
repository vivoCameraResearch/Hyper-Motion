        ...
        for ie, e in enumerate(edges):
            if e[0] < num_points and e[1] < num_points:
                ...
                if x1 > eps and y1 > eps and x2 > eps and y2 > eps:
                    color = mpl_colors.hsv_to_rgb([ie / float(len(edges)), 1.0, 1.0]) * 255
                    color = tuple(map(int, color))
                    cv2.line(canvas, (x1, y1), (x2, y2), color, thickness=line_thickness)

        for x, y in peaks:
            if x > eps and y > eps:
                cv2.circle(canvas, (int(x * W), int(y * H)), point_radius, (0, 0, 255), thickness=-1)
...
def process_video(video_path, body_json_path, hand_json_path, face_json_path, output_dir):
    ...
    hand_ann = {ann['frame_id']: ann.get('instances', []) for ann in hand_data}
    ...
    for frame_id in range(frame_count):
        ...
        # Hands
        hand_instances = hand_ann.get(frame_id, [])
        all_hand_peaks = []
        for instance in hand_instances:
            keypoints = instance.get('keypoints', [])
            if len(keypoints) >= 21:
                hand_peaks = [[x, y] for x, y in keypoints[:21]]
                all_hand_peaks.append(hand_peaks)
...
def main():
    ...
    parser.add_argument('--hand_json_dir', type=str, required=True)
    ...
    for video_file in video_files:
        ...
        hand_json = os.path.join(args.hand_json_dir, video_name + '_hand.json')
        face_json = os.path.join(args.face_json_dir, video_name + '_face.json')

        if all(os.path.exists(p) for p in [body_json, hand_json, face_json]):
            process_video(video_path, body_json, hand_json, face_json, args.output_dir)
        else:
